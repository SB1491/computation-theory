...
\end{pf*}
\section{NL = coNL}
NP와 coNP가 같은지는 밝혀지지 않았다. 그럼 NL과 coNL은 어떨까? 
\begin{thm}
    NL $=$ coNL이다. 
\end{thm}
\begin{pf*}
...
\begin{defn}
    계산 가능한 함수 $f: \mathbb{N} \rightarrow \mathbb{N}$에 대해 $f(n)$는 최소 $O(\log n)$이라고 하자.\footnote{고상한 표현으로 $f(n) = \Omega(\log n)$이라고 쓸 수도 있다.} $f$가 $O(f(n))$만큼의 공간만 사용하여 문자열 $1^n$을 $f(n)$의 이진수 표현으로 바꿔주면 $f$를 \textbf{공간 건설 가능한 함수}(space constructible function)라고 한다. 
\end{defn}
\begin{ex}
