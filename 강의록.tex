...
\begin{pf*}
    일단은 생략.
    먼저 TQBF가 PSPACE임은 예제 \ref{TQBF PSPAPCE}에서 보였으므로 넘어간다. 이제 TQBF가 PSPACE-하드임을 보이자. 먼저 편의를 위해 언어 $A$가 튜링 기계 $M$에 의해 공간 $n^k$를 사용하여 계산된다고 가정하자. 
\end{pf*}
\section{L과 NL}
...
\end{oq}
\section{NL = coNL}
앞에서 언급했듯 NP와 coNP가 같은지는 아직 밝혀지지 않았다. 그럼 NL과 coNL은 어떨까? 이는 놀랍게도 같다.
\begin{thm}
    NL $=$ coNL이다. 
\end{thm}
\begin{pf*}
