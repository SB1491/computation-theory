...
다행히도 이는 모두 결정 가능하다.
\begin{thm}
    다음과 같이 정의된 언어 $L_{\text{DFA}}$는 결정 가능하다.
    \begin{align*}
...
\begin{thm}
    \label{empty DFA}
    다음과 같이 정의된 언어 $E_{\text{DFA}}$는 결정 가능하다.
    \begin{align*}
...
\begin{thm}
    ...
    다음과 같이 정의된 언어 $EQ_{\text{DFA}}$는 결정 가능하다.
    \begin{align*}
...
\begin{pf*}
    ...
    정리 \ref{empty DFA}에 의해 $L(C)$는 결정 가능하므로, $EQ_{\text{DFA}}$는 
    결정 가능하다.
\end{pf*}
즉, 정규 언어에 관련된 것들은 전부 결정 가능하다. 그럼 이제 문맥무관 언어에 대해서도
알아보자.
\begin{thm}
    다음과 같이 정의된 언어 $L_{\text{CFG}}$는 결정 가능하다.
    \begin{align*}
...
\end{pf*}
\begin{thm}
    다음과 같이 정의된 언어 $E_\text{CFG}$는 결정 가능하다.
    \begin{align*}
...
정규 언어처럼 결정 가능할 것 같지만, 실제로는 그렇지 않다. 정리 \ref{DFA equiv decidable}에서
...
\section{co-NP}
\begin{defn}
    언어 $A$에 대해 $A^C \in \text{NP}$이면 $A$는 \textbf{co-NP}라고 한다.  
\end{defn}
\section{NP-완비}
