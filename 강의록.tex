...
중요한 연산은 바로 $\beta$-연산이다. 이제 이러한 간단한 연산을 이용해
아주 다양한 계산을 할 수 있음을 보이자.
\begin{defn}
...
\begin{pf*}
    ...
    \begin{enumerate}
        \item  먼저 $\langle M \rangle \in L(M)$이라고 하자. 이때 $L_d$의 정의 상 $\langle M \rangle \notin L$이어야
        한다. 즉, 모순이다.
        \item  먼저 $\langle M \rangle \in L(M)$이라고 하자. 이때 $L_d$의 정의 상 $\langle M \rangle \notin L$이어야
        한다. 즉, 모순이다. 
    \end{enumerate}
...
\end{pf*}
왜 재귀 열거 언어가 아닌 언어가 존재할까? 이는 튜링 기계가 유한하기 때문이다. 튜링 기계는 부호화가 가능하고,
이를 사전 순으로 나열하는 것이 가능하다. 즉, 자연수 집합과 일대일 대응이 가능하므로 크기가 자연수 집합과 같다.
그러나 언어는 $2^{\{0,1\}}$의 원소이다. 즉, 언어 집합의 크기는 실수 집합과 같다. 
...
\section{촘스키 위계}
잠시 계산에서 벗어나 우리가 지금까지 다뤄왔던 언어로 돌아와보자. 지금까지 다뤄왔던 많음 모델들 중
