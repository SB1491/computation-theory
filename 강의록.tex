...
\begin{defn}
    그래프 $G$와 그래프 내의 모든 정점 $n_1, n_2, n_3$에 대해 간선 $(n_1, n_2)$와 $(n_2, n_3)$의 가중치의 합이 $(n_1, n_3 )$의 가중치보다 크거나 같으면 그래프 $G$가 \textbf{삼각 부등식}을 만족한다고 한다. 
\end{defn}
...
\end{thm}
\begin{pf*}
    만약 $p$배를 넘지 않는 해를 보장하는 다항식 시간 근사 알고리즘 $A$가 존재한다고 가정하자. 
\end{pf*}
\section{연습문제}
