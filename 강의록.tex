...
\end{ec}
\begin{ec}
    언어 $A$의 \textbf{순서 뒤집기}(rotational closure)는 다음과 같이 정의된다. 
    \begin{align*}
        RC(A) = \{yx\;\vert\; xy \in A\}
    \end{align*}
    \begin{enumerate}
        \item 모든 언어 $A$에 대해, $RC(A) = RC(RC(A))$임을 보여라.
        \item $A$가 정규 언어면 $RC(A)$도 정규 언어임을 보여라.
    \end{enumerate}
\end{ec}
\begin{ec}
    다음과 같은 언어 $L$을 생각하자.
    \begin{align*}
        L= \{a^i b^j c^k \;\vert\; i, j, k \ge 0 \text{이고, } i = 1\text{이면 } j=k\text{이다.}\}
    \end{align*}
    \begin{enumerate}
        \item $L$이 펌프 정리를 만족함을 보여라. 
        \item $L$은 정규 언어인지 아닌지 증명해라.
    \end{enumerate}
\end{ec}
\chapter{문법과 오토마타} 
\section{문법}
...
\end{ec}
\begin{ec}
    $C$가 문맥무관 언어, $R$이 정규 언어라고 하자. $C \cap R$이 문맥무관 언어임을 보여라.
\end{ec}
\begin{ec}
    다음과 같은 언어 $L$을 생각하자. 
    \begin{align*}
        L = \{a^n b^n c^m d^m \;\vert\; n, m \ge 1\} \cup \{a^n b^m c^m d^n \;\vert\; n,m \ge 1\}
    \end{align*}
    \begin{enumerate}
        \item $L$이 문맥무관 언어임을 보여라. 
        \item $L$이 본질적으로 애매함을 보여라.  
    \end{enumerate}
\end{ec}
\begin{ec}
    다음과 같은 언어 $L$을 생성하는 문법 $G$를 구하라.\footnote{$N_a(w)$는 스트링 $w$에서 $a$의 개수를 의미한다.}
    \begin{align*}
...
\section{연습문제}
\begin{ec}
    다음 언어 $L$을 받아들이는 1-테입 튜링 기계를 제시하라.\footnote{$N_a(w)$는 스트링 $w$에서 $a$의 개수를 의미한다.}
    \begin{align*}
        L = \{w \;\vert\; w \in \{0,1\}^*, N_0(w) > N_1(w) \}
    \end{align*}
\end{ec}
\begin{ec}
    이진수를 입력 받으면 일진수로 반환하여 출력하는 1-테입 튜링 기계를 제시하라. 예를 들어, 1011을 입력 받으면 11111111111를 출력한다.
\end{ec}
\begin{ec}
    다음과 같은 \textbf{제한된 튜링 기계}를 생각하자. 정의 \ref{tmdefn}에서의 튜링 기계의 헤드는 총 3가지의 움직임을 할 수 있었다.\footnote{왼쪽, 오른쪽, 가만히 있기} 제한된 튜링 기계에서는 헤드가 매 스텝마다 가만히 있지 못하고 무조건 왼쪽이나 오른쪽으로 움직여야 한다고 하자. 그럼 제한된 튜링 기계의 게산 능력은 원래 튜링 기계의 계산 능력과 동등한가? 이를 밝혀라.
\end{ec}
\begin{ec}
    내리누름 오토마타는 하나의 스택만을 갖고 있었다. \textbf{$k$-스택 기계}($k$-스택 기계)는 스택이 $k$ 개 있는 결정론적 내리누름 오토마타이다. 한 번의 스텝에서 $k$-스택 기계는 (1) 새로운 상태로 바꾸고 (2) 각 스택에 삽입 도는 삭제 연산을 할 수 있다. $k$-스택 기계의 전이 규칙을 엄밀하게 작성하면 다음과 같다. 
    \begin{align*}
        \delta(q, a, X_1, \ldots, X_n) = (p, Y_1, \ldots, Y_n)
    \end{align*} 
    $2$-스택 기계의 계산 능력은 튜링 기계와 동등함을 보여라.
\end{ec}
\begin{ec}
    $k$-\textbf{계수기 기계}(counter machine)는 $k$-스택 기계와 비슷한 구조를 가지고 있다. $k$-스택 기계와 다른 점은 각 스택이 스택이 아니라 `계수기'라는 점이다. 각 계수기는 음이 아닌 정수를 저장할 수 있고, 우리는 계수기의 값이 0인지 아닌지만을 확인할 수 있다. 한 번의 스텝에서 (1) 새로운 상태로 바꾸고 (2) 각 계수기에서 1을 더하거나 뺄 수 있다. 이때 0이 저장된 계수기에서 1을 빼는 연산은 고려하지 않는다.
    \begin{enumerate}
        \item 모든 재귀 열거 언어 $L$에 대해 $L$을 받아들이는 3-계수기 기계가 존재함을 보여라. 
        \item 모든 재귀 열거 언어 $L$에 대해 $L$을 받아들이는 2-계수기 기계가 존재함을 보여라. 
    \end{enumerate}
\end{ec}
\begin{ec}
...
우리는 지금까지 튜링 기계의 뛰어난 계산 능력에 대해 알아보았다. 튜링 기계는 정말 다양한 일을 할 수 있으며, 이러한 기초적인 개념에서 확장되어 현재 우리가 사용하는 컴퓨터가 되었다. 그런데 튜링 기계는 정말 모든 일을 할 수 있을까? 혹시 튜링 기계가 못하는 일이 있을까? 정말 아쉽게도 튜링 기계가 하지 못하는 일은 존재한다. 자, 이제 컴퓨터 과학의 시작이자 하이라이트를 맛볼 준비가 되었다.
\section{튜링 기계의 부호화}
먼저 튜링 기계를 0과 1만으로 잘 표현하는 방법에 대해서 알아보자. 다음 절에서 튜링 기계의 설계도를 입력으로 넣기 위해서 이런 작업을 한다. 혹시 이런 세세하고 귀찮은 과정에 관심이 없다면, `아니 튜링 기계는 테입 빼면 적당히 유한한데 당연히 알잘딱하게 되는거 아님???'라고 생각하고 넘어가도 무방하다.\\ 
...
\begin{defn}\label{TM encoding}
    ...
    \begin{enumerate}
        ...
        \item $\delta(q,a) = (p, b, d)$는 각 $q,a,p,b,d$의 부호들 사이에 $1$을 두고 차례로 나열한다. 즉, $\delta(q_1, 0) = (q_2, 1, L)$은 01010010010이다. 
...
\end{pf*}
$BB(n)$, $FF(n)$는 너무 빠르게 증가해서 계산 불가능한 거라면, $pp(n)$은 오히려 너무 느리게 증가해서 계산 불가능하다.
\begin{thm}
...
\section{연습문제}
\begin{ec}
    \textbf{포스트의 일치 문제}(Post Correspondence Problem, PCP)는 다음과 같이 정의된다. $P$는 다음과 같은 도미노의 집합으로 구성된다.
    \begin{align*}
        P = \left\{\left[ \frac{t_1}{b_1} \right], \ldots ,\left[\frac{t_k}{b_k}\right] \right\}
    \end{align*}
    $P$의 \textbf{짝맞음}(match)은 $t_{i_1} \ldots t_{i_n} = b_{i_1} \ldots b_{i_n} $인 수열 $i_1, \ldots , i_n$이다. PCP는 $P$의 짝맞음이 존재하는지 결정하는 문제이다.
    \begin{align*}
        \text{PCP} = \{ \langle P \rangle \;\vert\; P \text{는 도미노의 집합이고 짝맞음이 존재한다.}\}
    \end{align*}
    \begin{enumerate}
        \item PCP가 결정 불가능함을 보이기 위해 새로운 문제인 PCP1을 정의하자.
        \begin{align*}
            \text{PCP1} = \{\langle P \rangle \; \vert \; P &\text{는 도미노들의 집합이고} 
            \\ & \text{짝맞음이 }P \text{의 첫 번째 도미노로 시작한다.} \}
        \end{align*}
        PCP1이 결정 불가능함을 보여라.
        \item PCP가 결정 불가능함을 보여라.
    \end{enumerate}
\end{ec}
...
\end{thm}
\begin{pf*}
    만약 $K$가 재귀면 $K$의 특성 함수인 $\chi_K  $가 존재한다. 그럼 다음 함수 $f$도 재귀여야 한다.
    \begin{align*}
        f (x) = 
        \begin{cases}
            \varphi_x (x) + 1 & x \in K \\ 
        ...
        \end{cases}
    \end{align*}
    그러나 모든 $x$에 대해 $f \neq \varphi_x$이므로 $f$는 재귀가 아니다.
\end{pf*}
\begin{thm}
    $K_0$는 재귀가 아니다.
\end{thm}
\begin{pf*}
    $x \in K$인 것과 $\langle x, x \rangle \in K_0$인 것은 동치이다. 즉, $K_0$가 특성함수를 가지면 $K$도 특성함수를 가지게 되므로 모순이다.
\end{pf*}
\begin{defn}
...
\end{thm}
\begin{pf*}
    모든 $y$에 대해 $\varphi_{e_0}$가 정의되지 않도록 하는 $e_0$를 고르자. 만약 $e_0 \in \overline{A}$이면 $K \le_1 A$이다. 반대고 $e_0 \in A$이면 $K \le_1 \overline{A}$이다. $A \neq \emptyset$이므로 $e_1 \in A$를 고를 수 있다. $A$는 색인 집합이므로 $\varphi_{e_1} \neq \varphi_{e_0}$이다. s-m-n 정리에 의해 다음과 같은 일대일 재귀 함수 $f$를 만들 수 있다.
    \begin{align*}
        \varphi_{f(x)}(y) = 
        \begin{cases}
    ...
    이제 다음을 알 수 있다.
    \begin{align*}
        x \in K \Rightarrow \varphi_{f(x)} = \varphi_{e_1} \Rightarrow f(x) \in A \\ 
        x \in \overline{K} \Rightarrow \varphi_{f(x)} = \varphi_{e_0} \Rightarrow f(x) \in \overline{A}
    \end{align*}
\end{pf*}
\begin{ex}
    $K \le_1 \text{Tot}, K \le_1 \overline{\text{Tot}}$임을 보여라.
\end{ex}
\begin{defn}
...
\end{ex}
\begin{ans*}
    먼저 $K_0$는 자명하게 1-완전하다. 그럼 $K_0 \le_1 K \le_1 K_1$을 보이자. 
    \begin{enumerate}
        \item $K_0 \le_1 K$: 
        다음과 같이 함수 $\psi (x, y, z)$를 정의하자. 
        \begin{align*}
            \psi(x, y, z) = \varphi_x (y) 
        \end{align*}
        s-m-n 정리에 의해 $\varphi_{f(x)}(y) = \psi(x, y, z)$인 일대일 재귀 함수 $f$가 존재한다.
        \item $K \le_1 K_1$:
        예제 \ref{KleTot}의 증명을 변형하면 자명하다.
    \end{enumerate}
\end{ans*}
...
\begin{pf*}
    ...
    \begin{enumerate}
        \item $s+1 = 2x + 1$인 경우: $h_s$는 일대일이고 $\mathrm{dom} h_s$는 유한하고 모든 $y \in \mathrm{dom} h_s$에 대해 $y \in A \Leftrightarrow h_s (y) \in B$가 성립한다고 하자. $h_s(x)$가 정의되면 아무것도 하지 않는다. $h_s(x)$가 정의되지 않으면
        \begin{align*}
            \{f(x), f(h_s^{-1} f(x)), \ldots,  f(h_s^{-1} f)^n (x), \ldots \}
        \end{align*}
        에서 $y$가 $h_s$의 치역에 없을 때까지 탐색한다. 이제 $h_{s+1} = y$로 정의할 수 있다.
...
\begin{ans*}
    \begin{align*}
        x \in \mathrm{Fin} &\Leftrightarrow \vert W_x \vert < \infty \\ 
        &\Leftrightarrow (\exists s) (\forall t) [t \le s \vee W_{x,t} = W_{x, s}]
    \end{align*}
\end{ans*}
\begin{ex}
...
\begin{ans*}
    \begin{align*}
        x \in \mathrm{Cof} &\Leftrightarrow \vert \overline{W_x}\vert < \infty \\ 
        &\Leftrightarrow (\exists y) (\forall z) (\exists s) [z \le y \vee z \in W_{x,s}]
    \end{align*}
\end{ans*}
...
\end{oq}
\section{최적화 문제}
현재까지 다룬 모든 문제는 Yes/No만이 답인 문제였다. 이런 문제를 \textbf{결정 문제}라고 한다. 그러나 우리가 다루는 문제가 항상 이런 것은 아니다. 현실세계에서 문제의 종류는 매우 다양하지만 보통 `최적해'를 찾는 \textbf{최적화 문제}(optimization problem)에 대해 다룬다.
\begin{defn}
    \textbf{(느슨한 정의)} 문제 A의 사례 $\alpha$를 문제 B의 사례 $\beta$로 바꾸되 다음 성질을 만족하면 다항식 시간 변환이라고 하고, 이를 $\alpha \le_{p} \beta$로 표기한다.
    \begin{enumerate}
        \item 변환은 다항 시간에 이루어진다.
        \item $\beta$의 답을 이용해서 $\alpha$의 답을 구할 수 있다.
    \end{enumerate}
\end{defn} 
우리가 지금까지 다뤄왔던 결정 문제를 통해 최적화 문제를 생각할 수 있을까? 간단하게 생각하면 예를 들어 SHORTPATH 문제에서 $k$를 미리 제시하는 것이 아닌 최소 $k$ 값이 얼마인지 묻는 문제를 생각하자. 이때는 $k$를 1부터 차례대로 검사하여 각각에 대해 결정 문제를 풀면 최적해를 찾아낼 수 있다.
\section{근사해 찾기}
우리가 아는 한 NP는 현재의 고전적인 컴퓨터로 정말 풀기 어려운 문제이다. 그럼 최소한 다항 시간 내에 풀리는 대충 적당히 좋은 해를 찾을 수 있는 방법은 없을까? 그마저도 꽤나 힘든 일이라는 것을 알아보자.
\begin{ex}
    언어 \textbf{TSP}는 다음과 같이 정의된다.
    \begin{align*}
        \text{TSP} = \{ \langle G, s, t, k \rangle \;\vert\; &G\text{는 가중치가 있는 그래프이고, }\\ 
        &s\text{에서 시작해서 } \\ 
        &t\text{에서 끝나는 헤밀토니안 경로 중} \\ 
        &\text{지나온 간선의 가중치 합이 }\\ 
        &k\text{보다 작은 경로가 존재한다.}\}
    \end{align*}
    HAMPATH가 NP-완전임을 이용해 TSP가 NP-완전임을 보여라.
\end{ex}
\begin{ans*}
    먼저 TSP가 NP임은 자명하다. \\
    이제 TSP가 NP-하드임을 보이기 위해 HAMPATH에 주어진 $G$를 적절히 $G'$로 변환하자. $G'$는 $G$와 정점과 간선이 모두 같지만, 모든 간선의 가중치가 1로 설정된 그래프이다. $k$의 값은 (모든 정점들의 개수 - 1)로 설정하면 TSP를 이용해 HAMPATH를 해결할 수 있다. 
\end{ans*}
TSP는 NP-완전이므로 매우 어려운 문제다. 그래서 근사적인 해를 찾으려고 하는데, 이마저도 매우 어렵다는 것을 보이자.
\begin{defn}
    그래프 $G$와 그래프 내의 모든 정점 $n_1, n_2, n_3$에 대해 간선 $(n_1, n_2)$와 $(n_2, n_3)$의 가중치의 합이 $(n_1, n_3)$의 가중치보다 크거나 같으면 그래프 $G$가 \textbf{삼각 부등식}(triangular inequality)을 만족한다고 한다. 
\end{defn}
만약 그래프가 삼각 부등식을 만족하면 TSP 문제는 근사하여 다항 시간 내에 풀 수 있다.\footnote{최대 신장 트리를 이용해 해결하는데, 구한 경로의 가중치의 합이 실제 최적해보다 최대 $\frac{3}{2}$배 크다.}
\begin{thm}
    $G$가 삼각 부등식을 만족하지 않는 가중치 그래프라고 하자. $\text{P} \neq \text{NP}$이면 모든 상수 $p$에 대해 최적해의 $p$배를 넘지 않는 해를 보장하는 다항식 시간 근사 알고리즘은 존재하지 않는다.
\end{thm}
\begin{pf*}
    만약 $p$배를 넘지 않는 해를 보장하는 다항식 시간 근사 알고리즘 $A$가 존재한다고 가정하자. 이제 HAMPATH가 $A$를 이용해 다항시간에 해결 가능함을 보일 것이다. \\ 
    HAMPATH의 $G$를 TSP 문제의 $G'$로 변환하는 과정을 생각하자. 전체 정점의 수를 $V$라고 할 때 정점들 간에 간선이 없는 경우 $G'$에서 $pV + 1$만큼의 가중치를 주고 간선으로 이어주자. 그럼 이제 $p$배를 넘지 않는 TSP 해가 존재한다는 것을 알면 원래 $G$에서 해밀토니언 경로가 존재한다는 것을 알 수 있다. 반대로 $p$배를 넘지 않는 TSP 해가 존재하지 않는다는 것을 알면 원래 $G$에서 해밀토니언 경로가 존재하지 않는다는 것을 알 수 있다. 따라서 HAMPATH를 다항 시간 내에 해결할 수 있다. 그러나 P $\neq$ NP라고 가정했으므로 이는 모순이다. 따라서 TSP의 $p$배를 넘지 않는 해를 보장하는 다항식 시간 근사 알고리즘은 존재하지 않는다.
\end{pf*}
\section{연습문제}
...
\end{ec}
\begin{ec}
    3-SAT이 NP-완전임을 이용해 HAMPATH가 NP-완전임을 보여라.
\end{ec}
\begin{ec}
    HAMPATH가 NP-완전임을 이용해 LONGPATH가 NP-완전임을 보여라.
\end{ec}
\chapter{공간 복잡도}
...
\end{ex}
\begin{ans*}
    그래프를 탐색할 때 예제 \ref{PATH}의 해설에서 썼던 방식은 공간을 $O(n)$만큼 사용하기 때문에 의미가 없다. 따라서 비결정론적인 방식으로 이를 탐색하고자 한다. 탐색을 할 때, 전체 경로가 아닌 현재 탐색하고 있는 정점만을 기록하면 된다. 이제 비결정론적으로 그 정점에서 간선을 따라 다른 정점을 탐색하고, 원래 있던 기록(출발했던 정점)은 지우고 현재 탐색하고 있는 정점으로 덧씌운다. 그래서 이 중에 최종 정점에 도달하게 되면 받아들이고, 정점들의 개수 이상으로 탐색을 많이 하면 받아들이지 않는다.
\end{ans*}
\begin{defn}
...
\begin{pf*}
    일단은 생략한다.
    정리 \ref{P reduction P}의 증명에서 사용한 방식을 하면 좋겠으나, 문제는 $f(w)$가 로그 공간보다 큰 공간을 차지할 수도 있어서 이를 그대로 사용할 수는 없다. 대신에, 
\end{pf*}
\begin{thm}
    PATH는 NL-완전이다.
\end{thm}
...
\begin{ex}
    ...
    \end{enumerate}
\end{ex}
독자들이 평범하게 알고 있는 대부분의 함수는 공간 건설 가능하다. 
\begin{thm}
...
\end{thm}
\begin{pf*}
    사비치 정리에 의해 $\text{NL} \subseteq \text{SPACE}(\log^2 n)$이므로 증명할 수 있다.
\end{pf*}
\begin{defn}
...
\begin{ex}
    ...
    \end{enumerate}
\end{ex}
공간 건설 가능한 함수처럼 독자들이 알고있는 평범한 함수는 전부 시간 건설 가능하다.
\begin{thm}
    \textbf{(시간적 위계 이론, time hierarchy theorem)} 시간 건설 가능한 함수 $t: \mathbb{N} \rightarrow \mathbb{N}$에 대해 $O(t(n))$에는 결정 가능하지만 $o(t(n)/\log n)$에는 결정 불가능한 언어 $A$가 존재한다.
\end{thm}
\begin{pf*}
    \textbf{(느슨한 증명)}
    $A$를 결정하는 알고리즘을 $D$라고 하자. $O(t(n))$에는 결정 가능하지만 $o(t(n)/\log n)$에는 결정 불가능하도록 하는 알고리즘 $D$는 다음과 같다. 입력은 $w$이고 $w$의 길이를 $n$이라고 하자.
    \begin{enumerate}
        \item $t(n)$은 시간 건설 가능한 함수이므로 $\lceil t(n) / \log t(n) \rceil$을 이진수로 저장할 수 있다. 앞으로 3과 4의 스텝을 진행할 때마다 이 값을 1씩 감소할 것이다. 만약 0에 다다르면 받아들이지 않는다. 
        \item $w$가 $\langle M \rangle 10^*$의 형식이 아니면 받아들이지 않는다.
        \item $M$을 입력 $w$에 대해 흉내낸다.
        \item $M$이 $w$를 받아들이면 받아들이지 않고, 받아들이지 않으면 받아들인다.
    \end{enumerate}
    먼저 $D$가 $O(t(n))$ 시간 안에 결정함을 보이자. 2에 의해 흉내내기의 횟수는 최대 $t(n) / \log t(n)$으로 제한되고, 그 길이는 최대 $\log ( t(n) / \log t(n)) = O(\log t(n))$이다. 따라서 값을 줄이는 데에 최대 $O(\log t(n))$의 시간이 걸릴 수 있으므로 총 시간 복잡도는 $O(t(n))$이 된다. \\ 
    이제 $A$가 $o(t(n) / \log t(n))$ 시간에 결정될 수 없음을 보이자. 이를 보이기 위해 먼저 $g(n)$ 시간에 $A$를 결정할 수 있는 튜링 기계 $M$이 존재한다고 가정하자. 이때 $g(n) = o(t(n) / \log t(n))$이다. 따라서 $D$는 어떤 상수 $d$에 대해 $d g(n)$ 시간에 $M$을 흉내낼 수 있다. $g(n)= o(t(n) / \log t(n))$이므로 적당히 큰 $n_0$에 대해 $n \ge n_0$이면 $dg(n) < t(n) / \log t(n)$을 만족한다. 따라서 입력을 $\langle M \rangle 1 0^{n_0}$으로 주면, 입력의 길이가 $n_0$보다 크므로 항상 흉내내기를 2에 영향 받지 않고 끝낼 수 있다. 이때 $D$는 $M$과 같은 입력에 대해 반대대되는 결과를 내뱉으므로 $M$은 $A$를 결정하지 않는다. 따라서 $A$는 $o(t(n) / \log t(n))$ 시간에 결정할 수 없다.
\end{pf*}
1-테입 튜링 기계에 한정하여 $o(t(n) / \log t(n))$와 같은 조건으로 위계를 나누었지만, 계산 모델을 수정하면 $o(t(n))$에 대해서도 같은 결과를 얻게 할 수도 있다.
\begin{thm}
    실수 $1 \le \epsilon_1 < \epsilon_2$에 대해 $\text{TIME}(n^{\epsilon_1}) \subsetneq \text{TIME}(n^{\epsilon_2})$이다. 
\end{thm}
\begin{defn}
    시간 복잡도 모임 EXPTIME은 다음과 같이 정의된다.
    \begin{align*}
...
\section{관계}
\begin{defn}
    집합 $A, B, R$에 대해 $R \subseteq A \times B$이면 $R$을 \textbf{관계}(relation)라고 한다. 이때, $R \subseteq A \times A$이면 $R$을 집합 $A$ 위의 관계라고 한다. $(a, b) \in R$이면 $a \;\, R \;\, b$라고 쓴다.
\end{defn}
\begin{ex}
...
\begin{defn}
    ...
    \begin{enumerate}
        ...
        \item $\forall a, b \in A, (a, b) \in R \rightarrow (b, a) \in R$이면 $R$을 \textbf{대칭적}(symmetric)이라고 한다.
        \item $\forall a, b, c\in A, (a, b) \in R \wedge (b, c) \in R \rightarrow 
        (a, c) \in R$이면 $R$을 \textbf{추이적}(transitive)이라고 한다.
        \item $R$이 반사적이고 대칭적이고 추이적이면 $R$을 \textbf{동치 관계}(equivalent relation)라고 한다.
    \end{enumerate}
\end{defn}
\begin{defn}
    관계 $R$의 \textbf{폐포}(closure) $C$란 $R \subseteq C$이면서 성질 $P$를 만족하는 최소의 관계이다. 예를 들어, 반사적 폐포, 대칭적 폐포, 추이적 폐포 등이 있다. 
\end{defn}
\begin{ex}
...
\begin{defn}
    ...
    두 집합 $X, Y$의 \textbf{기수}(cardinality) (또는 크기)가 같다고 한다. 이는 다음과 같이 적는다.
    \begin{align*}
        \vert X \vert = \vert Y \vert
    \end{align*}
\end{defn}
\begin{defn}
    $f: X \rightarrow Y$인 일대일 함수가 존재하면 집합 $Y$의 기수가 $X$보다 크거나 같다고 한다. 이는 다음과 같이 적는다.
    \begin{align*}
        \vert X \vert \le \vert Y \vert
    \end{align*} 
    만약 $\vert X \vert \le \vert Y \vert$인데 $\vert X \vert \neq \vert Y \vert$이면 $\vert X \vert < \vert Y \vert$라고 적는다.
\end{defn}
\begin{thm}
