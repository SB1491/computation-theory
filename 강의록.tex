...
\end{ex}
\begin{defn}
    집합 $A$가 어떤 부분 재귀 함수의 정의역이면 $A$를 재귀 열거라고 한다.
\end{defn}
\begin{defn}    
...
\end{ans*}
\begin{defn}
    집합 $A, B$에 대해 $p(A) = B$인 재귀적 순열 $p$가 존재하면 $A$와 $B$는 \textbf{재귀적 동형}(recursively isomorphic)이라고 한다. 이는 $A \equiv B$라고 쓴다. 
\end{defn}
\begin{thm}
...
\begin{pf*}
    ...
    $(\Leftarrow)$를 생각하자. 함수 $f$를 통해 $A \le_1 B$, $g$를 통해 $B \le_1 A$를 안다고 하자.
\end{pf*}
\section{튜링 위계}
...
\end{thm}
\begin{defn}
    $x, y, e < s, s > 0$이고 $\{\; e\; \}^A (x) = y$가 $\hat{P}_e$에서 $s$ 스텝보다 작은 스텝으로 계산되고 계산에서 $s$보다 작은 숫자들인 $z < s$만 사용되면 $\{\; e\; \}^A_s (x) = y$라고 적는다.
\end{defn}
\begin{defn}
    \begin{enumerate}
...
\section{산술적 위계}
\begin{defn}
    집합 $B$가 재귀적이면 $\Sigma_0^0 (\Pi_0^0)$ 안에 있다고 한다. 이를 $B \in \Sigma_0^0 (\Pi_0^0)$이라고 쓴다.
\end{defn}
\begin{defn}
    \begin{enumerate}
        \item $n \ge 1$에 대해 다음을 만족하는 재귀적 관계 $R$이 존재하면 $B$가 $\Sigma_n^0$ 안에 있다고 한다.
        \begin{align*}
            x \in B \Leftrightarrow (\exists y_1) (\forall y_2) (\exists y_3) \ldots (Q y_n) \; R(x, y_1, \ldots, y_n)
        \end{align*}
        $n$이 홀수면 $Q = \exists$, $n$이 짝수면 $Q = \forall$이다. 
        \item $n \ge 1$에 대해 다음을 만족하는 관계 $R$이 존재하면 $B$가 $\Pi_n^0$ 안에 있다고 한다.
        \begin{align*}
            x \in B \Leftrightarrow (\forall y_1) (\exists y_2)(\forall y_3) \ldots (Q y_n) \; R(x, y_1, \ldots, y_n)
        \end{align*}
