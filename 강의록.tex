...
\item Harry R. Lewis의 Elements of the Thoery of Computation
...
\end{ex}
\section{튜링 기계의 확장}
우리가 지금까지 다뤄본 튜링 기계는 굉장히 계산 능력이 뛰어나다. 
\section{비결정론적 튜링기계}
...
\begin{defn} \textbf{(naive한 정의)}
    ...
    충분히 다양한 연산이 가능하다.)
    레지스터는 0, 프로그램 카운터는 1로 초기화 된다. $\kappa$의 값은 몇 번째 인스트럭션을 수행해야하는지를 가르킨다.
...
\end{defn}
우리가 어떤 알고리즘(튜링 머신)의 시간 복잡도를 계산할때 입력(입력 스트링)의 크기 $n$에 따라 정확히
