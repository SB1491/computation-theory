...
\begin{pf*}
    ...
    \begin{enumerate}
        \item $s+1 = 2x + 1$: $h_s$는 일대일이고 $\mathrm{dom} h_s$는 유한하고 모든 $y \in \mathrm{dom} h_s$에 대해 $y \in A \Leftrightarrow h_s (y) \in B$가 성립한다고 하자. $h_s(x)$가 정의되면 아무것도 하지 않는다. $h_s(x)$가 정의되지 않으면  
        \item $s+1 = 2x + 2$: 
    \end{enumerate}
